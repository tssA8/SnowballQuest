...
Open [the iOS release workflow](https://github.com/tssA8/SnowballQuest/actions/workflows/ios-release.yml), choose **Run workflow**, select branch `feature/native-swift-ios` for the Swift app, choose a mode, and set an unused build number from 1 through 9999. Build `3` was the previous web-based app. Native version `0.3.0`, build `4`, was accepted and enabled for the existing internal TestFlight group on 2026-09-15; see [the successful release](https://github.com/tssA8/SnowballQuest/actions/runs/34935674826) and [verification record](VERIFICATION.md). Do not upload build `4` again. Check current App Store Connect builds before selecting the next unused number, at least `5`. The marketing version comes from `package.json` and the iOS project and must match.
