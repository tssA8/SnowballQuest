...
Open [the iOS release workflow](https://github.com/tssA8/SnowballQuest/actions/workflows/ios-release.yml), choose **Run workflow**, select branch `feature/native-swift-ios` for the Swift app, choose a mode, and set an unused build number from 1 through 9999. Build `3` was the previous web-based app; build `4` was native `0.3.0`. Native version **`0.3.1`, build `5`**, was accepted and enabled for the existing internal TestFlight group on 2026-09-15; see [the successful release](https://github.com/tssA8/SnowballQuest/actions/runs/34949159347) and [verification record](VERIFICATION.md). Do not upload builds `3`, `4` or `5` again. Check current App Store Connect builds before selecting the next unused number, at least `6`; the workflow input must be changed from its saved default `5`. The marketing version comes from `package.json` and the iOS project and must match.
